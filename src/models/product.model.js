...
async function list({ page = 1, limit = 10, nome, dataInicio, dataFim } = {}) {
  const safePage = Math.max(Number(page) || 1, 1);
  const safeLimit = Math.min(Math.max(Number(limit) || 10, 1), 100);
  const offset = (safePage - 1) * safeLimit;
  const { whereSql, values, nextIdx } = buildListQuery({ nome, dataInicio, dataFim });

  const { rows: items } = await pool.query(
    `SELECT id, nome, descricao, quantidade, unidade, data_cadastro, atualizado_em
     FROM produtos
     ${whereSql}
     ORDER BY nome
...
async function create(data) {
  const { rows } = await pool.query(
    'INSERT INTO produtos (nome, descricao, quantidade, unidade) VALUES ($1, $2, $3, $4) RETURNING id',
    [data.nome, data.descricao || null, data.quantidade, data.unidade]
  );
...
async function update(id, data) {
  ...
  for (const key of ['nome', 'descricao', 'quantidade', 'unidade']) {
